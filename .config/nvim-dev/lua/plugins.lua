...
-- vim.fn.stdpath 'data' is where vim stores data files, generally ~/.local/share/nvim
local is_bootstrap = false
if vim.fn.empty(vim.fn.glob(install_path)) > 0 then
  is_bootstrap = true
  vim.fn.execute('!git clone https://github.com/wbthomason/packer.nvim ' .. install_path)
  vim.cmd [[packadd packer.nvim]]
  -- this (packer) is the only plugin which you have to manually setup on vim's runtime path
  -- rest all the plugins are managed by packer itself
end
...
require('packer').startup(function(use)
  ...
  use {
    ...
    config = function()
      require('myconfig.lspconfig').setup()
      require('myconfig.custom_autoformat').setup()
    end,
  }
